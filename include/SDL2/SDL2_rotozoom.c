	...
	*/
	...
	for (y = 0; y < dst->h; y++) {
		...
		for (x = 0; x < dst->w; x++) {
			...
			/* Trace out src box and accumulate */
			oosp=sp;
			ra=ga=ba=aa=0;
...
\param numClockwiseTurns Number of clockwise 90 degree turns to apply to the src.
...
SDL_Surface* rotateSurface90Degrees(SDL_Surface* src, int numClockwiseTurns) 
{
	...
	if (!src || 
	    !src->format) {
		SDL_SetError("NULL src surface or src surface format");
	    return NULL; 
	}

	if ((src->format->BitsPerPixel % 8) != 0) {
		SDL_SetError("Invalid src surface bit depth");
	    return NULL; 
	}
	...
	/* If turns are even, our new width/height will be the same as the src surface */
...
\param width The src surface width.
\param height The src surface height.
	...
	double *canglezoom, double *sanglezoom)
{
	...
	/*
	* Determine destination width and height by rotating a centered src box
	*/
...
SDL_Surface *rotozoomSurfaceXY(SDL_Surface * src, double angle, double zoomx, double zoomy, int smooth)
{
	...
	*/
	...
	/*
	* Determine if src surface is 32bit or 8bit
	*/
	is32bit = (src->format->BitsPerPixel == 32);
	if ((is32bit) || (src->format->BitsPerPixel == 8)) {
		/*
		* Use src surface 'as is'
		*/
		rz_src = src;
		src_converted = 0;
	} else {
		/*
		* New src surface is 32bit with a defined RGBA ordering
		*/
		rz_src =
	...
	*/
	if (fabs(angle) > VALUE_LIMIT) {
		...
		*/
		...
		if (is32bit) {
			/*
			* Target surface is 32bit with src RGBA/ABGR ordering
			*/
			rz_dst =
...
\param width The width of the src surface to zoom.
\param height The height of the src surface to zoom.
...
the src box RGBA or Y information. If the surface is not 8bit
or 32bit RGBA/ABGR it will be converted into a 32bit RGBA format on the fly.
