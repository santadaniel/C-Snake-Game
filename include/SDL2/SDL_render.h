 ...
 *
 * When this texture is rendered, during the copy operation each src color
 ...
 *
 * When this texture is rendered, during the copy operation the src alpha
 ...
 *
 * \param texture the texture to update
 * \param alpha the src alpha value multiplied into copy operations
 * \returns 0 on success or a negative error code on failure; call
 *          SDL_GetError() for more information.
 *
 * \since This function is available since SDL 2.0.0.
 ...
 *
 * \param renderer the rendering context
 * \param texture the src texture
 * \param srcrect the src SDL_Rect structure or NULL for the entire texture
 ...
 * \param texture The src texture.
 * \param srcrect A pointer to the src rectangle, or NULL for the entire
 *                texture.
...
/**
 * Copy a portion of the src texture to the current rendering target, with
